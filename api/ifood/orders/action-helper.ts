// Helper unificado e autossuficiente para ações de pedidos iFood em Vercel Serverless
...
/**
 * Obtém tokens iFood do estabelecimento no Supabase
 */
export async function obterTokensIFoodEstabelecimento(estabelecimentoCodigo?: string, orderId?: string) {
  ...
  if (!targetCode && orderId) {
    try {
      ...
      console.warn("[iFood Action Enc Check Warn]", e);
    }
  }
  ...
  if (targetCode) {
    try {
      ...
      console.warn("[iFood Action TargetCode Check Warn]", tErr);
    }
  }
  ...
  try {
    ...
    console.warn("[iFood Action CD-5411 Check Warn]", cdErr);
  }
  ...
  try {
    ...
    console.warn("[iFood Action All Stores Check Warn]", gErr);
  }
...
export async function executarAcaoPedidoIFood(
  ...
  try {
    ...
    const estData = await obterTokensIFoodEstabelecimento(payload?.estabelecimento_codigo, cleanOrderId);
    ...
    if (acao === "confirm") {
      ifoodUrl = `https://merchant-api.ifood.com.br/order/v1.0/orders/${encodeURIComponent(cleanOrderId)}/confirm`;
    } else if (acao === "dispatch") {
      ifoodUrl = `https://merchant-api.ifood.com.br/order/v1.0/orders/${encodeURIComponent(cleanOrderId)}/dispatch`;
    } else if (acao === "cancel") {
      ifoodUrl = `https://merchant-api.ifood.com.br/order/v1.0/orders/${encodeURIComponent(cleanOrderId)}/requestCancellation`;
    ...
    console.log(`[iFood Action Request] Executando '${acao}' para pedido ${cleanOrderId} no iFood...`);
    ...
    if (acao === "cancel" && !res.ok && (res.status === 404 || res.status === 405)) {
      const fallbackUrl = `https://merchant-api.ifood.com.br/order/v1.0/orders/${encodeURIComponent(cleanOrderId)}/cancel`;
    ...
    try {
      await supabase
        ...
        .or(`codigo_pedido_ifood.eq.${cleanOrderId},id.eq.${cleanOrderId}`);
    } catch (dbErr) {
    ...
    console.error(`[iFood Action Fatal Exception - ${acao}]`, err);
    return {
      success: false,
      error: err?.message || "Erro inesperado ao processar ação no iFood.",
      status: 500,
    };
  }
}
