// Vercel Serverless Function: POST /api/ifood/orders/cancel
...
async function obterTokensIFood(estabelecimentoCodigo?: string, orderId?: string) {
  ...
  if (!targetCode && orderId) {
    try {
      ...
      console.warn("[iFood Cancel Enc Check Warn]", e);
    }
  }
  ...
  if (targetCode) {
    try {
      ...
      console.warn("[iFood Cancel TargetCode Check Warn]", tErr);
    }
  }
  ...
  try {
    ...
    console.warn("[iFood Cancel CD-5411 Check Warn]", cdErr);
  }
  ...
  try {
    ...
    console.warn("[iFood Cancel All Stores Check Warn]", gErr);
  }
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const reason = String(req.body?.reason || req.query?.reason || "Cancelado pelo estabelecimento").trim();
    const cancellationCode = String(req.body?.cancellationCode || req.query?.cancellationCode || "501").trim();
    ...
    let ifoodUrl = `https://merchant-api.ifood.com.br/order/v1.0/orders/${encodeURIComponent(orderId)}/requestCancellation`;
    ...
    console.log(`[iFood Cancel] Solicitando cancelamento do pedido ${orderId} no iFood...`);
    let ifoodRes = await fetch(ifoodUrl, { method: "POST", headers, body: cancelPayload });

    // Auto-refresh se 401
    if (ifoodRes.status === 401 && estData.refreshToken) {
      try {
        console.log(`[iFood Cancel 401] Renovando access_token para ${estData.codigo}...`);
        accessToken = await renovarToken(estData.id, estData.refreshToken);
        headers.Authorization = `Bearer ${accessToken}`;
        ifoodRes = await fetch(ifoodUrl, { method: "POST", headers, body: cancelPayload });
      } catch (renewErr) {
        console.error("[iFood Cancel Renew Token Error]", renewErr);
      }
    }

    // Fallback se requestCancellation retornar 404 (tenta /cancel direto)
    if (ifoodRes.status === 404) {
      const fallbackUrl = `https://merchant-api.ifood.com.br/order/v1.0/orders/${encodeURIComponent(orderId)}/cancel`;
      console.log(`[iFood Cancel] Tentando fallback para ${fallbackUrl}...`);
      const fallbackRes = await fetch(fallbackUrl, { method: "POST", headers, body: cancelPayload });
      if (fallbackRes.ok || fallbackRes.status === 202 || fallbackRes.status === 200 || fallbackRes.status === 204) {
        ifoodRes = fallbackRes;
      }
    }

    if (!ifoodRes.ok && ifoodRes.status !== 202 && ifoodRes.status !== 200 && ifoodRes.status !== 204) {
      const errBody = await ifoodRes.text();
      console.error(`[iFood Cancel Error] Falhou (HTTP ${ifoodRes.status}): ${errBody}`);
    ...
    try {
      ...
      console.warn("[iFood Cancel DB Status Warn]", dbErr);
    }

    return res.status(200).json({
      success: true,
      message: "Cancelamento do pedido processado com sucesso no iFood!",
    });
  } catch (err: any) {
    console.error("[iFood Cancel Fatal Exception]", err);
