...
async function obterTokensIFood(estabelecimentoCodigo?: string, orderId?: string) {
  ...
  if (!targetCode && orderId) {
    try {
      ...
      console.warn("[iFood Dispatch Enc Check Warn]", e);
    }
  }
  ...
  if (targetCode) {
    try {
      ...
      console.warn("[iFood Dispatch TargetCode Check Warn]", tErr);
    }
  }
  ...
  try {
    ...
    console.warn("[iFood Dispatch CD-5411 Check Warn]", cdErr);
  }
  ...
  try {
    ...
    console.warn("[iFood Dispatch All Stores Check Warn]", gErr);
  }
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const ifoodUrl = `https://merchant-api.ifood.com.br/order/v1.0/orders/${encodeURIComponent(orderId)}/dispatch`;
    const headers: Record<string, string> = {
      Authorization: `Bearer ${accessToken}`,
      Accept: "application/json",
    };

    console.log(`[iFood Dispatch] Despachando pedido ${orderId} no iFood...`);
    let ifoodRes = await fetch(ifoodUrl, { method: "POST", headers });

    // Auto-refresh se 401
    if (ifoodRes.status === 401 && estData.refreshToken) {
      try {
        console.log(`[iFood Dispatch 401] Renovando access_token para ${estData.codigo}...`);
        accessToken = await renovarToken(estData.id, estData.refreshToken);
        headers.Authorization = `Bearer ${accessToken}`;
        ifoodRes = await fetch(ifoodUrl, { method: "POST", headers });
      } catch (renewErr) {
        console.error("[iFood Dispatch Renew Token Error]", renewErr);
      }
    }

    if (!ifoodRes.ok && ifoodRes.status !== 202 && ifoodRes.status !== 200 && ifoodRes.status !== 204) {
      const errBody = await ifoodRes.text();
      console.error(`[iFood Dispatch Error] Falhou (HTTP ${ifoodRes.status}): ${errBody}`);
    ...
    try {
      ...
      console.warn("[iFood Dispatch DB Status Warn]", dbErr);
    }

    return res.status(200).json({
      success: true,
      message: "Pedido despachado com sucesso no iFood!",
    });
  } catch (err: any) {
    console.error("[iFood Dispatch Fatal Exception]", err);
