// Vercel Serverless Function for 99Food Order Cancellation (/api/99food/orders/cancel)
...
export default async function handler(req: any, res: any) {
  ...
  const cancellationCode = req.body?.cancellationCode || req.query?.cancellationCode;
  ...
  const result = await executarAcaoPedido99Food(orderId, "cancel", {
    reason,
    cancellationCode,
