// Vercel Serverless Function for Mercado Pago Webhook (/api/webhook-mp)
    ...
    if (req.method === "POST") {
      ...
      try {
        ...
        if (mpRes.ok) {
          ...
          console.warn(`[MercadoPago API Warn] Consulta ao pagamento #${paymentId} retornou HTTP ${mpRes.status}.`);
        }
      } catch (fetchErr: any) {
