  ...
  async fetch(request: Request, env: unknown, ctx: unknown) {
    try {
      ...
      if (url.pathname === "/api/validate-promo" && request.method === "POST") {
        try {
          ...
          if (cupomEncontrado) {
            ...
            const valorComDesc = isDias ? 0 : parseFloat((10.90 * ((100 - perc) / 100)).toFixed(2));
            return new Response(
              JSON.stringify({
                valido: true,
                cupom: cupomDigitado,
      ...
      if (url.pathname === "/api/afiliados" && request.method === "GET") {
        try {
          const supabaseUrl = process.env.VITE_SUPABASE_URL || "https://whfrjoqolyatylcwccon.supabase.co";
          const supabaseKey = process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.VITE_SUPABASE_ANON_KEY || "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6IndoZnJqb3FvbHlhdHlsY3djY29uIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODcwMzAzMTYsImV4cCI6MjEwMjYwNjMxNn0.km5zbjt0ZchneApZvVXzjdkYWS44CMZWwaLRz8nSeyY";
        ...
        try {
          ...
          if (planId === "anual") {
            ...
            if (!checkAntiFraude.elegivel) {
              ...
              transaction_amount = 10.90;
            } else if (cupomEnviado || (formData.transaction_amount && Number(formData.transaction_amount) < 10.90) || (payload.valor && Number(payload.valor) < 10.90)) {
              transaction_amount = Number(formData.transaction_amount || payload.valor || 10.90);
            } else {
              transaction_amount = 10.90;
            }
          }
