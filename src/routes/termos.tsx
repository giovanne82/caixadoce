...
function TermosDeUsoComponent() {
  const [activeTab, setActiveTab] = useState<"privacidade" | "termos">("termos");
    ...
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-slate-950 text-slate-900 dark:text-slate-100 py-8 px-4 sm:px-6">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="bg-white dark:bg-slate-900 p-4 sm:p-5 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:block">
              ...
              <p className="text-[11px] text-muted-foreground">Termos de Uso &amp; Condições de Serviço</p>
            </div>
          </div>
          ...
          <Tabs value={activeTab} onValueChange={(val) => setActiveTab(val as any)} className="w-full space-y-6">
            <div className="flex items-center justify-between border-b pb-4 flex-wrap gap-3">
              ...
              <span className="text-[11px] font-mono text-slate-400 flex items-center gap-1">
                <Lock className="w-3 h-3 text-purple-500" /> Acesso Público • Google Play Verified
              </span>
            </div>
        ...
        <div className="text-center text-xs text-slate-400 space-y-1 py-4">
          ...
          <p>Dúvidas legais? Fale conosco: <a href="mailto:contato@caixadoce.com.br" className="text-purple-600 font-bold hover:underline">contato@caixadoce.com.br</a></p>
        </div>
      </div>
    </div>
  );
...
export const Route = createFileRoute("/termos")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Termos de Uso e Condições de Serviço da plataforma CaixaDoce." },
    ],
  }),
