    ...
    <div className="py-12 px-6 text-center max-w-xl mx-auto space-y-5 bg-card border-2 border-dashed border-purple-500/40 rounded-3xl shadow-xl my-8">
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs font-bold text-purple-700 dark:text-purple-300">
          Escolha entre o Plano Mensal Completo (R$ 10,90/mês) ou Anual Completo para liberar o acesso ilimitado.
        </p>
      </div>
      ...
      if (data && Array.isArray(data)) {
        const mapeadas: Encomenda[] = data.map((d: any) => {
          ...
          const historicoMapeado = Array.isArray(histRaw) && histRaw.length > 0
            ? histRaw.map((p: any) => {
                ...
                return {
                  ...
                  data: p.data || p.date || String(d.created_at || "").split("T")[0] || d.data_entrega,
            ...
            ? [{
                id: "pay_initial",
                data: String(d.created_at || "").split("T")[0] || d.data_entrega,
          ...
          return {
            ...
            dados_brutos: d.dados_brutos || undefined,
    ...
    try {
      ...
      if (data && Array.isArray(data)) {
        const mapeadas: DespesaNotaFiscal[] = data.map((d: any) => ({
          ...
          dataCompra: d.data_compra || (d.created_at ? String(d.created_at || "").split("T")[0] : undefined) || new Date().toISOString().split("T")[0],
    ...
    try {
      ...
      if (data && Array.isArray(data)) {
        const mapeadas: ListaCompras[] = data.map((d: any) => ({
          ...
          estabelecimentoCodigo: d.estabelecimento_codigo,
          nome: d.nome || d.name,
          data: d.data || (d.created_at ? String(d.created_at || "").split("T")[0] : undefined) || new Date().toISOString().split("T")[0],
  ...
  const reenviarFinanceiro = async (despesa: DespesaNotaFiscal) => {
    ...
    try {
      ...
      await adicionarTransacao({
        ...
        data: despesa.dataCompra ? String(despesa.dataCompra || "").split("-").reverse().join("/") : new Date().toLocaleDateString("pt-BR"),
