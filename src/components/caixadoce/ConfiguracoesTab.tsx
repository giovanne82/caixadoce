...
export function ConfiguracoesTab({ onIrParaPlano }: ConfiguracoesTabProps) {
  ...
  const handleConectarIFood = async () => {
    ...
    try {
      const url = `/api/ifood/auth?estabelecimento_codigo=${encodeURIComponent(activeCode)}`;
      ...
      if (res.ok && data.success) {
        if (data.userCode) {
          ...
          const targetUrl = data.verificationUrlComplete || data.verificationUrl || `https://portal.ifood.com.br/autorizacao?code=${data.userCode}`;
          setUrlVerificacao(targetUrl);
          if (targetUrl) {
            window.open(targetUrl, "_blank");
          }
          toast.success(`Código de verificação gerado: ${data.userCode}. Autorize no portal do iFood.`);
        } else if (data.connected || data.accessToken) {
          toast.success("Sua loja foi conectada ao iFood!");
  ...
  const checarMpStatus = useCallback(async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from("estabelecimentos")
        .select("id, mp_access_token, mp_user_id, mp_public_key, usar_mercadopago, chave_pix_manual, modo_venda")
        .ilike("codigo", targetCode)
        .maybeSingle();
  ...
  const MENU_ITEMS = [
    {
      id: "perfil",
      title: "Meu Perfil",
      description: "Dados do Usuário & Assinatura Digital",
      icon: UserCheck,
    ...
    },
    {
      id: "loja",
      title: "Minha Loja",
      description: "Dados do Estabelecimento, Link do Cardápio, Endereço e CPF/CNPJ",
      icon: Store,
    ...
    },
    {
      id: "redes",
      title: "Redes Sociais",
      description: "WhatsApp, Instagram, TikTok e Facebook",
      icon: Share2,
    ...
    },
    {
      id: "pagamentos",
      title: "Pagamentos",
      description: "Gerenciador de Chaves Pix & Mercado Pago",
      icon: CreditCard,
    ...
    },
    {
      id: "frete",
      title: "Frete & Entrega",
      description: "Taxas de entrega, retirada e raio de atendimento",
      icon: Truck,
    ...
    },
    {
      ...
      description: "Assinatura Pro, Painel Afiliado e Painel Admin",
      icon: Crown,
    ...
    },
    {
      id: "seguranca",
      title: "Segurança",
      description: "Troca de senha e exclusão de conta",
      icon: Lock,
