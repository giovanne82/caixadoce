...
export function MeuPlanoTab() {
  ...
  // Checkout Mercado Pago Bricks State
  const [checkoutMPConfig, setCheckoutMPConfig] = useState<{
    open: boolean;
    planoId: string;
    nomePlano: string;
    valor: number;
  ...
  const handleSucessoMercadoPago = () => {
    salvarDadosPlanoEstabelecimento(activeCode, {
      planoId: "ilimitado",
      status: "ativo",
    });
    if (updateEstablishmentPlan) {
      updateEstablishmentPlan("ilimitado" as any, true);
    }
    recarregarPlano();
    setCheckoutMPConfig(null);
    toast.success("🎉 Parabéns! Seu plano Ilimitado foi ativado com sucesso!");
  };

  const handleAbrirMercadoPago = (
    planoId: string = "mensal",
    nomePlano: string = "Plano Mensal Completo PRO",
    valor: number = 19.90
  ) => {
    setCheckoutMPConfig({
      open: true,
  ...
  return (
    <div className="space-y-6">
      ...
      {/* COMPONENTE DO MERCADO PAGO CHECKOUT BRICKS (SE ABERTO) */}
      {checkoutMPConfig?.open ? (
        <MercadoPagoBrick
          estabelecimentoCodigo={activeCode}
          userEmail={user?.email || "contato@caixadoce.com.br"}
          planoId={checkoutMPConfig.planoId}
          nomePlano={checkoutMPConfig.nomePlano}
          valor={checkoutMPConfig.valor}
          onSuccess={handleSucessoMercadoPago}
          onCancel={() => setCheckoutMPConfig(null)}
        />
      ) : (
        <>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-white/90">
                  Garanta acesso ilimitado a todas as ferramentas: <strong>Plano Mensal Completo por apenas R$ 19,90/mês</strong> via Cartão ou Pix sem fidelidade.
                </p>
              </div>
            </div>
            <Button
              onClick={() => handleAbrirMercadoPago("mensal", "Plano Mensal Completo PRO", 19.90)}
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                ...
                <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
                  {infoPlano.status === "ativo" && infoPlano.planoId !== "basico" ? (
                    ...
                    <Button
                      onClick={() => handleAbrirMercadoPago("mensal", "Plano Mensal Completo PRO", 19.90)}
                      className="font-extrabold shadow-md bg-[#8E7CC3] hover:bg-[#7C69B3] text-white w-full sm:w-auto text-xs py-5"
                    >
                      <CreditCard className="w-4 h-4 mr-2" />
                      Assinar Mercado Pago (Cartão / Pix R$ 19,90)
                    </Button>
                  )}
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2 items-stretch">
            ...
            <Card className="border-2 border-[#8E7CC3] shadow-2xl relative flex flex-col justify-between bg-card hover:scale-[1.01] transition-all">
              ...
              <CardHeader className="pb-4 pt-6">
                ...
                <div className="pt-3">
                  ...
                  <p className="text-[11px] font-bold text-[#7C3AED] mt-0.5">
                    Preço promocional • Cartão de Crédito ou Pix
                  </p>
                </div>
              </CardHeader>
              <CardContent className="space-y-3">
              ...
              <div className="p-6 pt-0">
                ...
                >
                  {infoPlano.planoId === "ilimitado" || (infoPlano.planoId === "mensal" && infoPlano.status === "ativo")
                    ? "Plano Ativo (Ilimitado)"
                    : "Pagar com Mercado Pago (Cartão / Pix)"}
