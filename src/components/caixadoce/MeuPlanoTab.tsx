...
export function MeuPlanoTab() {
  ...
  const handleValidarCupom = async (e?: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/validate-promo", {
        ...
        body: JSON.stringify({ cupom: codigoLimpo, estabelecimentoCodigo: activeCode }),
      });
  ...
  const valorPlanoMensalComDesconto = useMemo(() => {
    ...
    return valorOriginalPlanoMensal;
  }, [cupomAplicado]);

  const valorOriginalPlanoAnual = 109.90;
  const valorPlanoAnualComDesconto = useMemo(() => {
    ...
    return valorOriginalPlanoAnual;
  }, [cupomAplicado]);
          ...
          <CardHeader className="pb-4 pt-6">
            ...
            <div className="pt-3">
                ...
                <div className="space-y-1">
                  {cupomAplicado.tipoDesconto === "dias_gratis" ? (
                    ...
                    <div className="space-y-0.5">
                      <div className="flex items-baseline gap-2">
                        ...
                        <span className="text-xs text-muted-foreground font-semibold"> / mês com cupom</span>
                      </div>
                    </div>
                  )}
                  <Badge variant="default" className="text-xs bg-emerald-600 hover:bg-emerald-600 text-white font-extrabold shadow-sm px-2.5 py-1">
                    {cupomAplicado.tipoDesconto === "dias_gratis"
                      ...
                      : `🎉 Cupom Aplicado! ${cupomAplicado.codigo} (R$ ${valorPlanoMensalComDesconto.toFixed(2).replace(".", ",")}/mês)`}
                  </Badge>
                </div>
              ) : (
                <div className="flex items-baseline gap-1.5">
              ...
              <p className="text-[11px] font-bold text-[#7C3AED] mt-1">
                Acesso total ilimitado • Cancele quando quiser
              </p>
            </div>
          </CardHeader>
          ...
          <CardHeader className="pb-4 pt-6">
            ...
            <div className="pt-3">
              {cupomAplicado ? (
                ...
                <div className="space-y-0.5">
                  <div className="flex items-baseline gap-1.5">
                    <span className="text-3xl font-black text-emerald-600">R$ 109,90</span>
                    <span className="text-xs text-muted-foreground font-semibold"> / ano</span>
                  </div>
                  <p className="text-[11px] text-muted-foreground font-mono">
                    <span className="line-through">De R$ 130,80</span> por R$ 109,90 (Economia de R$ {economiaEmReaisAnual.toFixed(2).replace(".", ",")}!)
                  </p>
                </div>
              )}
